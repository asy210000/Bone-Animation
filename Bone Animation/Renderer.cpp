...
{
	// Use find_if and a lambda to find the "cube" object.
	auto it = std::find_if(obj_list.begin(), obj_list.end(),
		[](const Object& obj) { return obj.obj_name == "cube"; });

	if (it == obj_list.end())
		return;  // "cube" object not found, exit early.
	...
	{
		glm::mat4 bone_obj_mat = glm::scale(m_bone_animation->bone_mat[i], m_bone_animation->scale_vector[i]);
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(bone_obj_mat));
		bone_obj->obj_color = m_bone_animation->colors[i];
		draw_object(shader, *bone_obj);
	}

	// Draw target
	glm::mat4 target_obj_mat = glm::translate(glm::mat4(1.0f), m_bone_animation->target);
	glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(target_obj_mat));
